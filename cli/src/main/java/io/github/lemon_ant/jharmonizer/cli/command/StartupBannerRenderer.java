 ...
 * The output is intended to be logged once at INFO level at the very start of a CLI run.
 */
@UtilityClass
class StartupBannerRenderer {
     ...
     * @param configDescription human-readable config source description
    ...
    static String render(
            ...
            @NonNull String configDescription,
    ...
    private static void addGlobRows(
        ...
        for (int index = 1; index < sortedGlobs.size(); index++) {
            lines.add(GLOB_INDENT + sortedGlobs.get(index));
        }
    }
}
