...
public class ProcessGroupAuditor extends NiFiAuditor {
     ...
     * @throws Throwable ex
     */
    @Around("within(org.apache.nifi.web.dao.ProcessGroupDAO+) && execution(void activateControllerServices(String,"
        + " org.apache.nifi.controller.service.ControllerServiceState, java.util.Collection<String>)) && args(groupId,"
        + " state, serviceIds)")
    public void activateControllerServicesAdvice(
     ...
     */
    @Around("within(org.apache.nifi.web.dao.ProcessGroupDAO+) && execution(org.apache.nifi.groups.ProcessGroup"
            + " createProcessGroup(String, org.apache.nifi.web.api.dto.ProcessGroupDTO))")
    ...
    @Around("within(org.apache.nifi.web.dao.ProcessGroupDAO+) && "
            ...
            + "args(groupId)")
    public ProcessGroup disconnectVersionControlAdvice(
            final ProceedingJoinPoint proceedingJoinPoint, final String groupId) throws Throwable {
     ...
     * @throws Throwable ex
     */
    @Around("within(org.apache.nifi.web.dao.ProcessGroupDAO+) && execution(void scheduleComponents(String,"
            + " org.apache.nifi.controller.ScheduledState, java.util.Set<String>)) && args(groupId, state,"
            + " componentIds)")
    public void scheduleComponentsAdvice(
            ProceedingJoinPoint proceedingJoinPoint, String groupId, ScheduledState state, Set<String> componentIds)
            throws Throwable {
        final Operation operation;
     ...
     */
    @Around("within(org.apache.nifi.web.dao.ProcessGroupDAO+) && execution(org.apache.nifi.groups.ProcessGroup"
            + " updateProcessGroup(org.apache.nifi.web.api.dto.ProcessGroupDTO)) && args(processGroupDTO)")
    public ProcessGroup updateProcessGroupAdvice(
            ProceedingJoinPoint proceedingJoinPoint, ProcessGroupDTO processGroupDTO) throws Throwable {
        ProcessGroupDAO processGroupDAO = getProcessGroupDAO();
        ...
        if (isAuditable()) {
            ...
            if (comments != null
                    && updatedProcessGroup.getComments() != null
                    && !comments.equals(updatedProcessGroup.getComments())) {
                // create the config details
                FlowChangeConfigureDetails configDetails = new FlowChangeConfigureDetails();
            ...
            if (parameterContext != null && updatedProcessGroup.getParameterContext() != null) {
                if (!parameterContext
                        .getIdentifier()
                        .equals(updatedProcessGroup.getParameterContext().getIdentifier())) {
                    // create the config details
                    FlowChangeConfigureDetails configDetails = new FlowChangeConfigureDetails();
                    configDetails.setName("Parameter Context");
                    configDetails.setValue(
                            updatedProcessGroup.getParameterContext().getIdentifier());
    ...
    public ProcessGroup updateVersionControlInformationAdvice(final ProceedingJoinPoint proceedingJoinPoint)
        ...
        final VersionControlInformationDTO vciDto =
                (VersionControlInformationDTO) proceedingJoinPoint.getArgs()[0];
    ...
    private Action generateUpdateConnectableAction(
            final Connectable connectable, final Operation operation, final Component component) {
    ...
    private Action generateUpdateRemoteProcessGroupAction(
            final RemoteProcessGroup remoteProcessGroup, final Operation operation) {
        final FlowChangeAction action = createFlowChangeAction();
    ...
    private List<Action> getComponentActions(
            final String groupId, final Collection<String> componentIds, final Operation operation) {
    ...
    private List<ControllerServiceNode> getControllerServices(final String groupId, final Collection<String> serviceIds)
            throws Throwable {
        final ProcessGroupDAO processGroupDAO = getProcessGroupDAO();
        final ProcessGroup processGroup = processGroupDAO.getProcessGroup(groupId);
    ...
    private void saveUpdateControllerServiceAction(final ControllerServiceNode csNode, final Operation operation)
            throws Throwable {
        final FlowChangeAction action = createFlowChangeAction();
