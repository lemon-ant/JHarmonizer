...
@CapabilityDescription(
        "Starts an HTTP Server and listens for HTTP Requests. For each request, creates a FlowFile and transfers to"
            + " 'success'. This Processor is designed to be used in conjunction with the HandleHttpResponse Processor"
            + " in order to create a Web Service. In case  of a multipart request, one FlowFile is generated for each"
            + " part.")
@WritesAttributes({
    @WritesAttribute(
            attribute = HTTPUtils.HTTP_CONTEXT_ID,
            description = "An identifier that allows the HandleHttpRequest and HandleHttpResponse "
    ...
    @WritesAttribute(
            attribute = "http.query.param.XXX",
            description = "Each of query parameters in the request will be added as an attribute, "
                    + "prefixed with \"http.query.param.\""),
    @WritesAttribute(
            attribute = "http.param.XXX",
            description =
                    "Form parameters in the request that are configured by \"Parameters to Attributes List\" will be"
                            + " added as an attribute, prefixed with \"http.param.\". Putting form parameters of large"
                            + " size is not recommended."),
    @WritesAttribute(
            attribute = HTTPUtils.HTTP_SSL_CERT,
            description = "The Distinguished Name of the requestor. This value will not be populated "
                    + "unless the Processor is configured to use an SSLContext Service"),
    @WritesAttribute(
            attribute = "http.issuer.dn",
            description =
                    "The Distinguished Name of the entity that issued the Subject's certificate. This value will not"
                            + " be populated unless the Processor is configured to use an SSLContext Service"),
    @WritesAttribute(
            attribute = "http.certificate.sans.N.name",
            description = "X.509 Client Certificate Subject Alternative Name value from mutual TLS authentication. The"
                    + " attribute name has a zero-based index ordered according to the content of Client"
                    + " Certificate"),
    @WritesAttribute(
            attribute = "http.certificate.sans.N.nameType",
            description = "X.509 Client Certificate Subject Alternative Name type from mutual TLS authentication. The"
                    + " attribute name has a zero-based index ordered according to the content of Client"
                    + " Certificate. The attribute value is one of the General Names from RFC 3280 Section"
                    + " 4.1.2.7"),
    @WritesAttribute(
    ...
    @WritesAttribute(
            attribute = "http.headers.multipart.XXX",
            description = "Each of the HTTP Headers that is received in the multipart request will be added as an"
                    + " attribute, prefixed with \"http.headers.multipart.\" For example, if the multipart request"
                    + " contains an HTTP Header named \"content-disposition\", then the value will be added to an"
                    + " attribute named \"http.headers.multipart.content-disposition\""),
    @WritesAttribute(
            attribute = "http.multipart.size",
            description =
                    "For requests with Content-Type \"multipart/form-data\", the part's content size is recorded into"
                            + " this attribute"),
    @WritesAttribute(
            attribute = "http.multipart.content.type",
            description =
                    "For requests with Content-Type \"multipart/form-data\", the part's content type is recorded into"
                            + " this attribute"),
    @WritesAttribute(
            attribute = "http.multipart.name",
            description =
                    "For requests with Content-Type \"multipart/form-data\", the part's name is recorded into this"
                            + " attribute"),
    @WritesAttribute(
            attribute = "http.multipart.filename",
            description =
                    "For requests with Content-Type \"multipart/form-data\", when the part contains an uploaded file,"
                        + " the name of the file is recorded into this attribute. Files are stored temporarily at the"
                        + " default temporary-file directory specified in \"java.io.File\" Java Docs)"),
    @WritesAttribute(
            attribute = "http.multipart.fragments.sequence.number",
            description =
                    "For requests with Content-Type \"multipart/form-data\", the part's index is recorded into this"
                            + " attribute. The index starts with 1."),
    @WritesAttribute(
            attribute = "http.multipart.fragments.total.number",
            description =
                    "For requests with Content-Type \"multipart/form-data\", the count of all parts is recorded into"
                            + " this attribute.")
})
...
public class HandleHttpRequest extends AbstractProcessor implements ListenComponent {
    ...
    public static final PropertyDescriptor ALLOW_PUT = new PropertyDescriptor.Builder()
            ...
            .defaultValue("true")
            .build();
    public static final AllowableValue CLIENT_NEED = new AllowableValue(
            "Need Authentication",
            "Need Authentication",
            "Processor will reject communications from any client unless the client provides a certificate that is"
                    + " trusted by the TrustStorespecified in the SSL Context Service");

    // Allowable values for client auth
    public static final AllowableValue CLIENT_NONE = new AllowableValue(
            "No Authentication",
            "No Authentication",
            "Processor will not authenticate clients. Anyone can communicate with this Processor anonymously");
    public static final AllowableValue CLIENT_WANT = new AllowableValue(
            "Want Authentication",
            "Want Authentication",
            "Processor will try to verify the client but if unable to verify will allow the client to communicate"
                    + " anonymously");
    public static final PropertyDescriptor CLIENT_AUTH = new PropertyDescriptor.Builder()
            .name("Client Authentication")
            .description(
                    "Specifies whether or not the Processor should authenticate clients. This value is ignored if the"
                            + " <SSL Context Service> Property is not specified or the SSL Context provided uses only a"
                            + " KeyStore and not a TrustStore.")
            .required(true)
    ...
    public static final PropertyDescriptor MULTIPART_READ_BUFFER_SIZE = new PropertyDescriptor.Builder()
            .description("The threshold size, at which the contents of an incoming file would be written to disk. Only"
                    + " applies for requests with Content-Type: multipart/form-data. It is used to prevent denial"
                    + " of service type of attacks, to prevent filling up the heap or disk space.")
    ...
    public static final PropertyDescriptor MULTIPART_REQUEST_MAX_SIZE = new PropertyDescriptor.Builder()
            ...
            .description(
                    "The max size of the request. Only applies for requests with Content-Type: multipart/form-data,"
                        + " and is used to prevent denial of service type of attacks, to prevent filling up the heap"
                        + " or disk space")
            .required(true)
            .addValidator(StandardValidators.DATA_SIZE_VALIDATOR)
            .defaultValue("1 MB")
    ...
    public static final PropertyDescriptor PATH_REGEX = new PropertyDescriptor.Builder()
            .name("Allowed Paths")
            .description("A Regular Expression that specifies the valid HTTP Paths that are allowed in the incoming URL"
                    + " Requests. If this value is specified and the path of the HTTP Requests does not match this"
                    + " Regular Expression, the Processor will respond with a 404: NotFound")
            .required(false)
    ...
    private final CertificateAttributesProvider certificateAttributesProvider =
            new HandleHttpRequestCertificateAttributesProvider();
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        if (Strings.CS.contains(request.getContentType(), MIME_TYPE__MULTIPART_FORM_DATA)) {
            final long requestMaxSize = context.getProperty(MULTIPART_REQUEST_MAX_SIZE)
                    .asDataSize(DataUnit.B)
                    .longValue();
            final int readBufferSize = context.getProperty(MULTIPART_READ_BUFFER_SIZE)
                    .asDataSize(DataUnit.B)
                    .intValue();
    ...
    protected void handleFlowContentStreamingError(
            final ProcessSession session,
            final HttpRequestContainer container,
            final Optional<FlowFile> flowFile,
            final Exception e) {
        ...
        getLogger()
                .error(
                        "Stream Processing Failed: Method [{}] URI [{}] Address [{}]",
                        request.getMethod(),
    ...
    synchronized void initializeServer(final ProcessContext context) throws Exception {
        ...
        this.containerQueue = new LinkedBlockingQueue<>(
                context.getProperty(CONTAINER_QUEUE_SIZE).asInteger());
        ...
        final int requestHeaderSize = context.getProperty(REQUEST_HEADER_MAX_SIZE)
                .asDataSize(DataUnit.B)
                .intValue();
        ...
        final String parametersToAttributesPropertyValue =
                context.getProperty(PARAMETERS_TO_ATTRIBUTES).getValue();
        ...
        final HttpServlet standardServlet = new HttpServlet() {
            ...
            protected void service(final HttpServletRequest request, final HttpServletResponse response) {
                ...
                if (added) {
                    getLogger()
                            .debug(
                                    "Request Queued: Method [{}] URI [{}] Address [{}]",
                                    method,
                                    requestUri,
                                    request.getRemoteAddr());
                } else {
    ...
    private void forwardFlowFile(
            final ProcessSession session, final long start, final HttpServletRequest request, final FlowFile flowFile) {
        ...
        getLogger()
                .debug("Transferred {} to [{}] Remote Address [{}] ", flowFile, REL_SUCCESS, request.getRemoteAddr());
    }
    ...
    private FlowFile savePartAttributes(
            ProcessSession session, Part part, FlowFile flowFile, final int i, final int allPartsCount) {
    ...
    private FlowFile saveRequestAttributes(
            final ProcessContext context,
            final ProcessSession session,
        ...
        final Map<String, String> certificateAttributes =
                certificateAttributesProvider.getCertificateAttributes(request);
    ...
    private void sendError(
        ...
        try {
            ...
            getLogger()
                    .warn(
                            "Send Error Completed: HTTP {} [{}] Method [{}] URI [{}] Address [{}]",
                            statusCode,
                            message,
                            method,
                            uri,
            ...
            getLogger()
                    .error(
                            "Send Error Failed: HTTP {} [{}] Method [{}] URI [{}] Address [{}]",
                            statusCode,
                            message,
                            method,
                            uri,
    ...
    private static class HttpRequestContainer {
        ...
        public HttpRequestContainer(
                final HttpServletRequest request, final HttpServletResponse response, final AsyncContext async) {
