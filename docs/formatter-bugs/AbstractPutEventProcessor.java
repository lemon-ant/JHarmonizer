...
public abstract class AbstractPutEventProcessor<T> extends AbstractSessionFactoryProcessor {
    ...
    public static final PropertyDescriptor IDLE_EXPIRATION = new PropertyDescriptor.Builder()
            ...
            .description("The amount of time a connection should be held open without being used before closing the"
                    + " connection. A value of 0 seconds will disable this feature.")
            .required(true)
    ...
    public static final PropertyDescriptor MAX_SOCKET_SEND_BUFFER_SIZE = new PropertyDescriptor.Builder()
            ...
            .description("The maximum size of the socket send buffer that should be used. This is a suggestion to the"
                    + " Operating System to indicate how big the socket buffer should be. If this value is set too"
                    + " low, the buffer may fill up before the data can be read, and incoming data will be"
                    + " dropped.")
    ...
    public static final PropertyDescriptor MESSAGE_DELIMITER = new PropertyDescriptor.Builder()
            ...
            .description(
                    "Specifies the delimiter to use for splitting apart multiple messages within a single FlowFile. If"
                        + " not specified, the entire content of the FlowFile will be used as a single message. If"
                        + " specified, the contents of the FlowFile will be split on this delimiter and each section"
                        + " sent as a separate message. Note that if messages are delimited and some messages for a"
                        + " given FlowFile are transferred successfully while others are not, the messages will be"
                        + " split into individual FlowFiles, such that those messages that were successfully sent are"
                        + " routed to the 'success' relationship while other messages are sent to the 'failure' "
                        + "relationship.")
            .required(false)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
    ...
    public static final PropertyDescriptor OUTGOING_MESSAGE_DELIMITER = new PropertyDescriptor.Builder()
            ...
            .description(
                    "Specifies the delimiter to use when sending messages out over the same TCP stream. The delimiter"
                        + " is appended to each FlowFile message that is transmitted over the stream so that the"
                        + " receiver can determine when one message ends and the next message begins. Users should"
                        + " ensure that the FlowFile content does not contain the delimiter character to avoid errors."
                        + " In order to use a new line character you can enter '\\n"
    ...
    protected EventSender<T> getEventSender(final ProcessContext context) {
        ...
        final boolean singleEventPerConnection =
                context.getProperty(CONNECTION_PER_FLOWFILE).getValue() != null
                        ? context.getProperty(CONNECTION_PER_FLOWFILE).asBoolean()
                        : false;
        ...
        factory.setSocketSendBufferSize(context.getProperty(MAX_SOCKET_SEND_BUFFER_SIZE)
        ...
        final int timeout = context.getProperty(TIMEOUT)
                .evaluateAttributeExpressions()
                .asTimePeriod(TimeUnit.MILLISECONDS)
                .intValue();
        final int idleTimeout = context.getProperty(IDLE_EXPIRATION)
                .evaluateAttributeExpressions()
    ...
    protected abstract NettyEventSenderFactory<T> getNettyEventSenderFactory(
    ...
    protected String getOutgoingMessageDelimiter(final ProcessContext context, final FlowFile flowFile) {
        String delimiter = context.getProperty(OUTGOING_MESSAGE_DELIMITER)
    ...
    protected class FlowFileMessageBatch {
        ...
        public synchronized void completeSession() {
            ...
            if (successfulRanges.isEmpty()) {
                getLogger()
                        .error(
                                "Failed to send {}; routing to 'failure'; last failure reason reported was {};",
                                flowFile,
                                lastFailureReason);
            ...
            if (failedRanges.isEmpty()) {
                ...
                session.getProvenanceReporter()
                        .send(flowFile, transitUri, "Sent " + successfulRanges.size() + " messages;", transferMillis);
                session.transfer(flowFile, REL_SUCCESS);
                getLogger()
                        .info(
                                "Successfully sent {} messages for {} in {} millis",
                                successfulRanges.size(),
                                flowFile,
                                transferMillis);
                session.commitAsync();
                return;
            ...
            // At this point, the successful ranges is not empty and the failed ranges is not empty. This indicates that
            // some messages made their way
            // successfully and some failed. We will address this by splitting apart the source FlowFile into children
            // and sending the successful messages to 'success'
            ...
            getLogger()
                    .error(
                            "Successfully sent {} messages, but failed to send {} messages; the last error received"
                                    + " was {}",
                            successfulRanges.size(),
                            failedRanges.size(),
                            lastFailureReason);
