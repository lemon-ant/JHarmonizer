...
### 2. Record-member ordering policies (future version)
...
### 3. Constants ordering expansion (future version)
...
### 4. Annotation ordering policies (future version)
...
### 5. Collapse single-value annotation array braces (future version)
...
### 6. Add a type-based selector to rule lines (field type / method return type)
...
### 7. JUnit 5 test visibility normalizer (future version)
...
### 8. Redundant Java modifier cleanup pass (future version)
...
### 9. Automatic constant naming convention enforcer (`UPPER_SNAKE_CASE`) (future version)
...
### 10. Override-aware parameter ordering harmonizer for methods and constructors (future version)
...
### 11. Automatic fully-qualified name optimizer (future version)
...
### 12. Git-aware changed-files processing mode (future version)
...
### 13. Adaptive static-import optimizer (future version)
...
### 14. Vendor-format configuration adapters (IntelliJ IDEA, Eclipse, EditorConfig, Spotless)
...
### 15. Flexible logging-level controls for CLI, wrappers, and public API (future version)
...
### 16. Static-candidate conversion analyzer (instance -> static) (future version)
...
### 17. Package dependency cycle detector (future version)
...
### 18. Visibility minimization pass (future version)
...
### 19. Nullability interfaces / annotations enforcer (future version)
...
### 20. Lombok policy enforcer and optimizer (future version)
...
### 21. Inter-procedural initializer dependencies (field default expression -> method calls)
...
### 22. Explicit-declaring-type instance forward-reference corner-case (parked failing E2E)
...
### 23. Compile group sorting once and precompute ordering rule values in `MemberDescriptor` (performance-focused)
