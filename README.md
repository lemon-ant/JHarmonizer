...
Check conformance (reports all violations, always exits 0):

```bash
mvn jharmonizer:check
```

Fail fast on first violation (exits non-zero — useful in CI):

```bash
mvn jharmonizer:check-fast
```
...
```bash
java -jar cli/target/jharmonizer-cli.jar reorder --base-dir src/main/java
java -jar cli/target/jharmonizer-cli.jar check-all --base-dir src/main/java
java -jar cli/target/jharmonizer-cli.jar check-fast --base-dir src/main/java
```

See [`cli/README.md`](cli/README.md) for the full CLI reference including all options, exit codes, and CI integration examples.
...
See [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) for community standards, and [CHANGELOG.md](CHANGELOG.md) for what is planned.
