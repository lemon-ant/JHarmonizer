...
public class AbstractDynamoDBIT {
    ...
    public static void oneTimeSetup() {
        ...
        final AttributeDefinition partitionKeyDefinition = AttributeDefinition.builder()
                .attributeName(PARTITION_KEY)
                .attributeType(ScalarAttributeType.S)
                .build();
        final KeySchemaElement partitionKeySchema = KeySchemaElement.builder()
                .attributeName(PARTITION_KEY)
                .keyType(KeyType.HASH)
                .build();
        final AttributeDefinition sortKeyDefinition = AttributeDefinition.builder()
                .attributeName(SORT_KEY)
                .attributeType(ScalarAttributeType.N)
                .build();
        final KeySchemaElement sortKeySchema = KeySchemaElement.builder()
                .attributeName(SORT_KEY)
    ...
    protected BatchGetItemResponse getBatchGetItems(int count, String table, boolean includeSortKey) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            partitionKey.put(
                    PARTITION_KEY,
                    AttributeValue.builder().s(PARTITION_KEY_VALUE_PREFIX + i).build());
            if (includeSortKey) {
                partitionKey.put(
                        SORT_KEY, AttributeValue.builder().n(String.valueOf(i)).build());
            }
    ...
    protected TestRunner initRunner(final Class<? extends Processor> processorClass) {
        ...
        runner.setProperty(
                AbstractDynamoDBProcessor.ENDPOINT_OVERRIDE,
