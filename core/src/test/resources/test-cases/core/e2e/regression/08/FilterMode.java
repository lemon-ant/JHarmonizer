...
public enum FilterMode implements DescribedValue {
    ...
    FILTER_MODE_FULL_PATH(
            ...
            "Filtering will be applied by evaluating the regular expression defined in " + FILE_FILTER.getDisplayName()
                    ...
                    + " is set to true, the entire subdirectory tree will be searched for files in which the full path"
                    + " of the file matches the regular expression defined in " + FILE_FILTER.getDisplayName()
    ...
    public static FilterMode forName(String filterMode) {
        return Stream.of(values())
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Invalid filter mode: " + filterMode));
    }
