...
final class EmbeddedQuestDbStatusHistoryRepositoryDefinitions {
    ...
    static final String COMPONENT_STATUS_QUERY = "SELECT * FROM %s " + "WHERE componentId = '%s' "
            + "AND captured > to_timestamp('%s', '"
            + CAPTURE_DATE_FORMAT + "') " + "AND captured < to_timestamp('%s', '"
            + CAPTURE_DATE_FORMAT + "') " + "ORDER BY captured ASC";
    ...
    static final RequestMapping<StandardStatusSnapshot> CONNECTION_STATUS_REQUEST_MAPPING =
            getSnapshotRequestMapping(ConnectionStatus.class, CONNECTION_METRICS.values());

    // Component counter

    static final String TABLE_NAME_COMPONENT_COUNTER = "componentCounter";
    static final String CREATE_COMPONENT_COUNTER =
            "CREATE TABLE " + TABLE_NAME_COMPONENT_COUNTER + " (" + "captured TIMESTAMP,"
    ...
    static final String CREATE_CONNECTION_STATUS =
            "CREATE TABLE " + TABLE_NAME_CONNECTION_STATUS + " (" + "captured TIMESTAMP,"
                    + "componentId SYMBOL capacity 2000 nocache index capacity 1500,"
                    + "inputBytes LONG,"
                    + "inputCount LONG,"
                    + "outputBytes LONG,"
    ...
    static final String CREATE_GARBAGE_COLLECTION_STATUS =
            "CREATE TABLE " + TABLE_NAME_GARBAGE_COLLECTION_STATUS + " (" + "captured TIMESTAMP,"
    ...
    static final String CREATE_NODE_STATUS = "CREATE TABLE " + TABLE_NAME_NODE_STATUS + " (" + "captured TIMESTAMP,"
            + "freeHeap LONG,"
            + "usedHeap LONG,"
    ...
    static final String CREATE_PROCESSOR_STATUS =
            "CREATE TABLE " + TABLE_NAME_PROCESSOR_STATUS + " (" + "captured TIMESTAMP,"
                    + "componentId SYMBOL capacity 2000 nocache index capacity 1500,"
                    + "bytesRead LONG,"
                    + "bytesWritten LONG,"
    ...
    static final String CREATE_PROCESS_GROUP_STATUS =
            "CREATE TABLE " + TABLE_NAME_PROCESS_GROUP_STATUS + " (" + "captured TIMESTAMP,"
                    + "componentId SYMBOL capacity 2000 nocache index capacity 1500,"
                    + "bytesRead LONG,"
                    + "bytesWritten LONG,"
                    + "bytesTransferred LONG,"
    ...
    static final String CREATE_REMOTE_PROCESS_GROUP_STATUS =
            "CREATE TABLE " + TABLE_NAME_REMOTE_PROCESS_GROUP_STATUS + " (" + "captured TIMESTAMP,"
                    + "componentId SYMBOL capacity 2000 nocache index capacity 1500,"
                    + "sentBytes LONG,"
                    + "sentCount LONG,"
                    + "receivedBytes LONG,"
    ...
    static final String CREATE_STORAGE_STATUS =
            "CREATE TABLE " + TABLE_NAME_STORAGE_STATUS + " (" + "captured TIMESTAMP,"
                    + "name SYMBOL capacity 256 nocache,"
                    + "storageType SHORT,"
                    + "freeSpace LONG,"
                    + "usedSpace LONG"
    ...
    static final String NODE_STATUS_QUERY = "SELECT * FROM nodeStatus " + "WHERE captured > to_timestamp('%s', '"
            + CAPTURE_DATE_FORMAT + "') " + "AND captured < to_timestamp('%s', '"
            + CAPTURE_DATE_FORMAT + "') " + "ORDER BY captured ASC";
    ...
    static final RequestMapping<StandardStatusSnapshot> PROCESSOR_STATUS_REQUEST_MAPPING =
            getSnapshotRequestMapping(ProcessorStatus.class, PROCESSOR_METRICS.values());
    ...
    static final RequestMapping<StandardStatusSnapshot> PROCESS_GROUP_STATUS_REQUEST_MAPPING =
            getSnapshotRequestMapping(ProcessGroupStatus.class, PROCESS_GROUP_METRICS.values());
    ...
    static final RequestMapping<StandardStatusSnapshot> REMOTE_PROCESS_GROUP_STATUS_REQUEST_MAPPING =
            getSnapshotRequestMapping(RemoteProcessGroupStatus.class, REMOTE_PROCESS_GROUP_METRICS.values());
    static final String STATUS_QUERY_GARBAGE_COLLECTION =
            "SELECT * FROM garbageCollectionStatus " + "WHERE captured > to_timestamp('%s', '"
                    + CAPTURE_DATE_FORMAT + "') " + "AND captured < to_timestamp('%s', '"
                    + CAPTURE_DATE_FORMAT + "') " + "ORDER BY captured ASC";
    static final String STORAGE_STATUS_QUERY = "SELECT * FROM storageStatus " + "WHERE captured > to_timestamp('%s', '"
            + CAPTURE_DATE_FORMAT + "') " + "AND captured < to_timestamp('%s', '"
            + CAPTURE_DATE_FORMAT + "') " + "ORDER BY captured ASC";
    ...
    static {
        ...
        REMOTE_PROCESS_GROUP_METRICS.put(
                6, RemoteProcessGroupStatusDescriptor.RECEIVED_BYTES_PER_SECOND.getDescriptor());
        ...
        REMOTE_PROCESS_GROUP_METRICS.put(
                9, RemoteProcessGroupStatusDescriptor.AVERAGE_LINEAGE_DURATION.getDescriptor());
    }
    ...
    static InsertRowDataSource getConnectionStatusDataSource(
            final Collection<CapturedStatus<ConnectionStatus>> statuses) {
        return new ComponentStatusDataSource<>(statuses.iterator(), CONNECTION_METRICS, ConnectionStatus::getId);
    }

    static InsertRowDataSource getCounterStatisticsDataSource(
            final Collection<CapturedStatus<ProcessorStatus>> statuses) {
        return CounterStatisticsDataSource.getInstance(statuses);
    }

    static InsertRowDataSource getGarbageCollectionStatusDataSource(
            final Collection<CapturedStatus<GarbageCollectionStatus>> statuses) {
    ...
    static InsertRowDataSource getProcessGroupStatusDataSource(
            final Collection<CapturedStatus<ProcessGroupStatus>> statuses) {
        return new ComponentStatusDataSource<>(statuses.iterator(), PROCESS_GROUP_METRICS, ProcessGroupStatus::getId);
    }

    static InsertRowDataSource getProcessorStatusDataSource(
            final Collection<CapturedStatus<ProcessorStatus>> statuses) {
        return new ComponentStatusDataSource<>(statuses.iterator(), PROCESSOR_METRICS, ProcessorStatus::getId);
    }

    static InsertRowDataSource getRemoteProcessGroupStatusDataSource(
            final Collection<CapturedStatus<RemoteProcessGroupStatus>> statuses) {
        return new ComponentStatusDataSource<>(
                statuses.iterator(), REMOTE_PROCESS_GROUP_METRICS, RemoteProcessGroupStatus::getId);
    }

    static QueryResultProcessor<Map<Long, Map<StandardMetricDescriptor<NodeStatus>, Long>>>
            getStorageStatusResultProcessor() {
        return new StorageStatusResultProcessor(NODE_STATUS_METRICS);
    }

    private static <T> RequestMapping<StandardStatusSnapshot> getSnapshotRequestMapping(
            Class<T> type, Collection<MetricDescriptor<T>> descriptorSource) {
        final RequestMappingBuilder<StandardStatusSnapshot> requestMappingBuilder = RequestMappingBuilder.of(
                        () -> new StandardStatusSnapshot(new HashSet<>(descriptorSource)))
                .addLongField(
                        (snapshot, field) -> snapshot.setTimestamp(new Date(TimeUnit.MICROSECONDS.toMillis(field))))
                .addStringField((snapshot, field) -> {}); // Id is not used
        descriptorSource.forEach(descriptor ->
                requestMappingBuilder.addLongField((snapshot, field) -> snapshot.addStatusMetric(descriptor, field)));
