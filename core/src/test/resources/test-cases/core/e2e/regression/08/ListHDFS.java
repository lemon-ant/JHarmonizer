...
@CapabilityDescription(
        "Retrieves a listing of files from HDFS. For each file that is listed in HDFS, this processor creates a"
            + " FlowFile that represents the HDFS file to be fetched in conjunction with FetchHDFS. This Processor is"
            + " designed to run on Primary Node only in a cluster. If the primary node changes, the new Primary Node"
            + " will pick up where the previous node left off without duplicating all of the data. Unlike GetHDFS,"
            + " this Processor does not delete any data from HDFS.")
@WritesAttributes({
    @WritesAttribute(attribute = "filename", description = "The name of the file that was read from HDFS."),
    @WritesAttribute(
            attribute = "path",
            description = "The path is set to the absolute path of the file's directory on HDFS. For example, if the"
                    + " Directory property is set to /tmp, then files picked up from /tmp will have the path"
                    + " attribute set to \"./\". If the Recurse Subdirectories property is set to true and a file"
    ...
    @WritesAttribute(
            attribute = "hdfs.lastModified",
            description =
                    "The timestamp of when the file in HDFS was last modified, as milliseconds since midnight Jan 1,"
                            + " 1970 UTC"),
...
@Stateful(
        ...
        description =
                "After performing a listing of HDFS files, the latest timestamp of all the files listed is stored."
                    + " This allows the Processor to list only files that have been added or modified after this date"
                    + " the next time that the Processor is run, without having to store all of the actual"
                    + " filenames/paths which could lead to performance problems. State is stored across the cluster"
                    + " so that this Processor can be run on Primary Node only and if a new Primary Node is selected,"
...
public class ListHDFS extends AbstractHadoopProcessor {
    ...
    public static final PropertyDescriptor FILE_FILTER_MODE = new PropertyDescriptor.Builder()
            ...
            .description("Determines how the regular expression in  " + FILE_FILTER.getDisplayName()
                    + " will be used when retrieving listings.")
            .required(true)
    ...
    public static final PropertyDescriptor MAXIMUM_FILE_AGE = new PropertyDescriptor.Builder()
            ...
            .addValidator(StandardValidators.createTimePeriodValidator(
                    100, TimeUnit.MILLISECONDS, Long.MAX_VALUE, TimeUnit.NANOSECONDS))
            .build();
    public static final PropertyDescriptor MINIMUM_FILE_AGE = new PropertyDescriptor.Builder()
            ...
            .required(false)
            .addValidator(StandardValidators.createTimePeriodValidator(
                    0, TimeUnit.MILLISECONDS, Long.MAX_VALUE, TimeUnit.NANOSECONDS))
            .build();
    public static final PropertyDescriptor RECORD_WRITER = new PropertyDescriptor.Builder()
            .name("Record Writer")
            .description(
                    "Specifies the Record Writer to use for creating the listing. If not specified, one FlowFile will"
                        + " be created for each entity that is listed. If the Record Writer is specified, all entities"
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            ...
            if (legacyLatestListingTimestampString != null) {
                ...
                latestTimestamp = legacyLatestListingTimestamp == legacyLatestEmittedTimestamp
                        ? legacyLatestListingTimestamp + 1
                        : legacyLatestListingTimestamp;
                latestFiles = new ArrayList<>();
                getLogger()
                        .debug(
                                "Transitioned from legacy state to new state. 'legacyLatestListingTimestamp': {},"
                                        + " 'legacyLatestEmittedTimeStamp': {}','latestTimestamp': {}",
                                legacyLatestListingTimestamp,
                                legacyLatestEmittedTimestamp,
                                latestTimestamp);
            ...
            getLogger()
                    .error("Failed to retrieve timestamp of last listing from the State Manager. Will not perform"
                            + " listing until this is accomplished.");
            context.yield();
            return;
        }
        ...
        getLogger()
                .debug(
                        "Found a total of {} files in HDFS, {} are listed",
                        fileStatusIterable.getTotalFileCount(),
                        writer.getListedFileCount());
    ...
    protected Collection<ValidationResult> customValidate(ValidationContext context) {
        ...
        if (minimumAge > maximumAge) {
            problems.add(new ValidationResult.Builder()
                    .valid(false)
                    .subject("ListHDFS Configuration")
                    .explanation(MINIMUM_FILE_AGE.getDisplayName() + " cannot be greater than "
                            + MAXIMUM_FILE_AGE.getDisplayName())
                    .build());
        }
        return problems;
    }
    ...
    protected void preProcessConfiguration(Configuration config, ProcessContext context) {
        ...
        // Since this processor is marked as INPUT_FORBIDDEN, the FILE_FILTER regex can be compiled here rather than
        // during onTrigger processing
    ...
    private PathFilter createPathFilter(final ProcessContext context) {
        ...
        return switch (filterMode) {
            case FILTER_MODE_FILES_ONLY ->
                path -> fileFilterRegexPattern.matcher(path.getName()).matches();
            case FILTER_MODE_FULL_PATH ->
                path -> fileFilterRegexPattern.matcher(path.toString()).matches()
                        || fileFilterRegexPattern
                                .matcher(Path.getPathWithoutSchemeAndAuthority(path)
                                        .toString())
                                .matches();
            // FILTER_DIRECTORIES_AND_FILES
            default ->
                path -> Stream.of(Path.getPathWithoutSchemeAndAuthority(path)
                                .toString()
                                .split("/"))
